...
async def get_users_settings(_, message):
    ...
    sudo_count = len(sudo_users) if sudo_users else 0
    auth_count = len(auth_chats) if auth_chats else 0
    activity_rate = (active_users / total_users * 100) if total_users > 0 else 0

    # Header with TamsHub branding
    msg = f"""👥 <b>TamsHub User Management System</b>
<i>Comprehensive User Analytics & Control</i>

📊 <b>User Statistics Overview:</b>
├ Total Users: {total_users}
├ Active Users: {active_users}
├ Sudo Users: {sudo_count}
├ Authorized Chats: {auth_count}
...
"""
    ...
    if user_data:
        for user_id, data in user_data.items():
            settings_count = len(data) if data else 0
            status_icon = "🟢" if data else "🔴"
            status_text = "Active" if data else "Inactive"

            msg += f"""👤 User {user_id}: {status_icon} {status_text}
├ Settings: {settings_count} configured
└─────────────────────

"""

    msg += f"""
🛠️ <b>Management Options:</b>
├ Use /auth [user_id] to authorize users
├ Use /unauth [user_id] to remove authorization
├ Use /addsudo [user_id] to add sudo access
└ Use /rmsudo [user_id] to remove sudo access

🔥 <b>TamsHub User Management - Professional Control</b>"""

    # Check if message is too long
    msg_ecd = msg.encode()
    if len(msg_ecd) > 4000:
        with BytesIO(msg_ecd) as ofile:
            ofile.name = "tamshub_users_settings.txt"
            await send_file(message, ofile)
    else:
